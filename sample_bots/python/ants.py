...
class Ants():
    ...
    def update(self, data):
        ...
        for (row, col), owner in list(self.ant_list.items()):
        ...
        for row, col in self.dead_list:
            self.map[row][col] = LAND
        self.dead_list = []
        for (row, col), owner in list(self.hill_list.items()):
            self.map[row][col] = LAND
        self.hill_list = {}
    ...
    def my_ants(self):
        return [loc for loc, owner in list(self.ant_list.items())
                    if owner == MY_ANT]

    def enemy_ants(self):
        return [(loc, owner) for loc, owner in list(self.ant_list.items())
                    if owner != MY_ANT]
    
    def my_hills(self):
        return [loc for loc, owner in list(self.hill_list.items())
                    if owner == MY_ANT]

    def enemy_hills(self):
        return [(loc, owner) for loc, owner in list(self.hill_list.items())
                    if owner != MY_ANT]
